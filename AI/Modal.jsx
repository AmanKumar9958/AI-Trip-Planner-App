...
if (!apiKey) {
  console.error(
    "Error: EXPO_PUBLIC_GROQ_API_KEY is not set. Please check your .env file.",
  );
}

const client = new OpenAI({
  apiKey: apiKey || "dummy-key", // Prevent crash if key is missing
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful assistant. Provide response in plain text.",
        },
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a travel assistant. You must return only strict JSON. Do not use markdown (```json). Do not add any conversational text.",
          },
          { role: "user", content: prompt },
        ],
        model: MODEL_NAME,
