...
export default function MyTrip() {
  ...
  const shareTrip = async (trip) => {
    try {
      ...
      const days = trip?.userSelection?.TotalDays
        ? `${trip.userSelection.TotalDays} Day(s)`
        : "";
      const budget = trip?.userSelection?.budget
        ? `Budget: ${trip.userSelection.budget}`
        : "";
      const withWho = trip?.userSelection?.TravelingWith
        ? `With: ${trip.userSelection.TravelingWith}`
        : "";
      const summary = [destination, days, budget, withWho]
        .filter(Boolean)
        .join(" • ");
