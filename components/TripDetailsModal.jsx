    ...
    return (
        ...
        >
            <View className="flex-1 bg-black/50 justify-end">
                <Animated.View 
                    style={[{transform: [{ translateY: pan.y.interpolate({
                        inputRange: [0, 1000],
                        outputRange: [0, 1000],
                        extrapolate: 'clamp'
                    }) }]}, pan.getLayout()]}
                ...
                <View {...panResponder.panHandlers} className="h-60 w-full relative">
                    ...
                    <Text className="text-gray-200">
                        {trip.userSelection?.TotalDays} Days • {trip.userSelection?.TravelingWith} • {trip.userSelection?.budget}
                    </Text>
                    </View>
                </View>
                ...
                <ScrollView className="flex-1 p-5" showsVerticalScrollIndicator={false}>
                    ...
                    {hotels.length > 0 && (
                    <View className="mb-6">
                        <Text className="text-xl font-bold text-black dark:text-white mb-4">Recommended Hotels</Text>
                        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        {hotels.map((hotel, index) => (
                            <View key={index} className="mr-4 w-60 bg-gray-50 dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-700">
                            <Image 
                                // Use a fallback image if specific hotel image is missing
                                source={{ uri: hotel?.ImageUrl || 'https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=1000&auto=format&fit=crop' }} 
                                className="w-full h-32"
                            />
                            <View className="p-3">
                                {/* FIX: Handle PascalCase keys often returned by AI */}
                                <Text className="font-bold text-lg text-black dark:text-white mb-1" numberOfLines={1}>
                                    {hotel.HotelName || hotel.hotel_name}
                                </Text>
                                <Text className="text-gray-500 dark:text-gray-400 text-xs mb-2" numberOfLines={2}>
                                    {hotel.HotelAddress || hotel.hotel_address}
                                </Text>
                                <View className="flex-row justify-between items-center">
                                <Text className="text-orange-500 font-bold">{hotel.Price || hotel.price}</Text>
                    ...
                    <View className="mb-2">
                        ...
                        */}
                        ...
                        <View key={index} className="mb-6 ml-4 border-l-2 border-orange-200 pl-6 relative">
